    ...
    <div class="col-md-12">
        ...
        <div class="portlet light portlet-fit portlet-form bordered">
            <div class="portlet-title">
                <div class="caption">
                    <span class="caption-subject font-red sbold uppercase">{{__('cp.edit_feature')}}</span>
                </div>
            </div>
            <div class="portlet-body">
                <!-- BEGIN FORM-->
                <form action="{{url('feature/update/'.$feature->id)}}" method="post" class="form-horizontal" novalidate="novalidate"  enctype="multipart/form-data">
                    @csrf
                    <div class="form-body">
                            ...
                            @foreach($locales as $locale)
                                <div class="form-group">
                                    <label class="control-label col-md-3 for="title_{{$locale->lang}}">
                                        {{_('cp.title_'.$locale->lang)}}
                                        <span class="required" aria-required="true">*</span>
                                    </label>
                                    <div class="col-md-6">
                                        <input type="text" class="form-control" name="title_{{$locale->lang}}" data-required="1"
                                               value="{{$feature->translate($locale->lang)->title}}" id="title_{{$locale->lang}}"
                                               required>
                                    </div>
                                </div>
                                <br>
                        ...
                        <fieldset>
                            @foreach($locales as $locale)
                                <div class="form-group">
                                    <label class="control-label col-md-3 for="title_{{$locale->lang}}">
                                        {{_('cp.description_'.$locale->lang)}}
                                        <span class="required" aria-required="true">*</span>
                                    </label>
                                    <div class="col-md-6">
                                        <input type="text" class="form-control" name="description_{{$locale->lang}}" data-required="1"
                                               value="{{$feature->translate($locale->lang)->description}}" id="description_{{$locale->lang}}"
                                               required>
                                    </div>
                                </div>
                                <br>
